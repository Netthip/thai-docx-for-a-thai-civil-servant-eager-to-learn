...
- ✅ **หนังสือราชการสำเร็จรูป**: บันทึกข้อความ + หนังสือภายนอก (วางโครงให้ตามแบบฟอร์ม)
...
## 📑 หนังสือราชการ: บันทึกข้อความ & หนังสือภายนอก

`create_docx()` คุม *การจัดหน้า* ส่วนโมดูล [`scripts/thai_official.py`](scripts/thai_official.py) วาง *โครงหนังสือราชการ* ให้ตามแบบฟอร์มจริง (ระเบียบงานสารบรรณ) — ส่วนราชการ / ที่ / วันที่ / เรื่อง / เรียน / อ้างถึง / สิ่งที่ส่งมาด้วย / เนื้อหา / คำลงท้าย / ลงชื่อ ใช้ helper ตัวเดิม ฟอนต์ไทยจึงถูกต้องทุกฝั่งและข้อความเต็มบรรทัดเหมือนกัน

### บันทึกข้อความ

```python
from thai_official import create_memo, thai_date
import datetime
...
create_memo(
    ...
    date=thai_date(datetime.date(2026, 7, 1)),   # → "๑ กรกฎาคม ๒๕๖๙"
    subject="ขออนุมัติเบิกค่าใช้จ่ายในการเดินทางไปราชการ",
    to="ผู้อำนวยการกองคลัง",
    body=[
        "ด้วยข้าพเจ้ามีความจำเป็นต้องเดินทางไปราชการ ...",
...
```python
from thai_official import create_letter, thai_date
import datetime

create_letter(
    "letter.docx",
    doc_number="ศธ ๐๒๐๑/๑๒๓",
    sender_lines=["สำนักงานเลขานุการกรม", "ถนนราชดำเนินนอก เขตดุสิต", "กรุงเทพมหานคร ๑๐๓๐๐"],
    date=thai_date(datetime.date(2026, 7, 1)),
    subject="ขอเชิญเข้าร่วมประชุมเชิงปฏิบัติการ",
    to="อธิการบดีมหาวิทยาลัยเชียงใหม่",
    enclosure="กำหนดการประชุม จำนวน ๑ ชุด",
    body=["ด้วยกรมมีกำหนดจัดประชุม ...", "จึงเรียนมาเพื่อโปรดพิจารณา"],
    closing="ขอแสดงความนับถือ",
    signer_name="นายสมชาย รักงาน",
    signer_position="เลขานุการกรม",
    contact_lines=["กองคลัง", "โทร. ๐ ๒๒๐๒ ๑๒๓๔"],
)
```

ดูตัวอย่างเต็มที่ [`examples/example_memo.py`](examples/example_memo.py) และ [`examples/example_letter.py`](examples/example_letter.py)

> **ตราครุฑ** เป็นตราแผ่นดิน รีโปนี้ไม่ฝังมาให้ — ระบุไฟล์รูปของหน่วยงานเองผ่าน `garuda_image="krut.png"` ถ้าไม่ใส่ จะเว้นที่ไว้ตามแบบฟอร์ม
> **เลขไทย**: ใส่ `thai_digits=True` เพื่อแปลงเลขในช่อง *ที่/วันที่* เป็นเลขไทย หรือใช้ `to_thai_digits("123")` → `"๑๒๓"` กับข้อความใดก็ได้
